...
def handler(event, context):
    ...
    functions = s3.get_object(Bucket=S3_BUCKET, Key=s3_prefix+'/'+event['FunctionsObject'])
    functions = json.loads(functions['Body'].read())
    ...
    esms = esms['EventSourceMappings']['EventSourceMappings']
